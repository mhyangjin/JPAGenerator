...
public class DBCoreGenEntityWriter extends DBCoreGenFileWriter {
	private static final String EntityHeader = 
	...
	private static final String EntityClassDefine=	
			"@Getter\r\n" + 
			"@Setter \r\n" + 
			"@ToString\r\n" + 
			"@Entity\r\n" +
			...
			"public class <CLASSNAME> implements DBCoreEntity {";
	
	private static  final String EntityColumnDefine="@Column(name=\"<COLUMNNAME>\")";
	private static  final String EntitykeyDefine=
											"	@Transient\r\n" + 
											"	private Key key;\r\n" + 
											"	\r\n" ;
	private static  final String keyClassDefine="	public static class Key extends DBCoreEntityKeyImpl {\r\n" ;
	...
	public static void generateFile(String path, String packageName, TableInfo tableInfo) {
		...
		try {
			...
			writeStream(bufferWriter, EntityHeader );
			//comment
			writeStream(bufferWriter, commentString );
			//class define
			String classDefine =  EntityClassDefine.replaceAll("<TABLENAME>", tableName)
			...
			writeStream(bufferWriter, EntitykeyDefine);
			//footer define
	...
	public static String[][] makeKeyColumns (List<ColumnInfo> keyColumnInfos) {
		...
		for ( ColumnInfo keyColumnInfo:keyColumnInfos)  {
			...
			keydefines[i][0] = "	@Id " + EntityColumnDefine.replaceAll("<COLUMNNAME>", columnName);
	...
	public static String[][] mekeColumns (List<ColumnInfo> columnInfos) {
		...
		for ( ColumnInfo columnInfo:columnInfos)  {			
			try {
				...
				columndefines[i][0] = "	" + EntityColumnDefine.replaceAll("<COLUMNNAME>", columnName);
				if ( dbClumnType.getJavaType().equals("Date")) {
					columndefines[i][0] =columndefines[i][0] +"\r\n	@Temporal(TemporalType.TIMESTAMP)";
					if ( columnName.toUpperCase().contains("CREATE"))
						columndefines[i][0] =columndefines[i][0] +"\r\n	@CreationTimestamp";
					if ( columnName.toUpperCase().contains("UPDATE"))
						columndefines[i][0] =columndefines[i][0] +"\r\n	@UpdateTimestamp";
				}	
